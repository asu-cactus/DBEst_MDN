...
class Query1:
    ...
    def build_model(self, mdl_name: str = "flight_1m", encoder="binary"):
        ...
        if self.model_size == "1m":
            ...
            self.sql_executor.execute("set n_epoch=2")  # 50
            self.sql_executor.execute("set n_gaussians_reg=8")  #
            self.sql_executor.execute("set n_gaussians_density=10")  # 8
            self.sql_executor.execute(
                "create table "
                + mdl_name
                + "(dep_delay real, distance real) from '../data/flights/flight_1m.csv' GROUP BY unique_carrier method uniform size 'num_points/flights_group.csv' "
            )
            ...
            self.sql_executor.execute("set n_epoch=2")  # 20
            self.sql_executor.execute("set n_gaussians_reg=8")  #
            self.sql_executor.execute("set n_gaussians_density=8")  # 10
            self.sql_executor.execute(
                "create table "
                + mdl_name
                + "(dep_delay real, distance real) from '../data/flights/flight_5m.csv' GROUP BY unique_carrier method uniform size 'num_points/flights_group.csv' "
            )
    ...
    def build_one_model(self, mdl_name: str = "flight_1m", encoder="binary"):
        ...
        if self.model_size == "1m":
            ...
            self.sql_executor.execute(
                "create table "
                + mdl_name
                + "(taxi_out real, distance real) from '../data/flights/flight_1m.csv' method uniform size 0.01"
            )
        elif self.model_size == "5m":
            ...
            self.sql_executor.execute(
                "create table "
                + mdl_name
                + "(taxi_out real, distance real) from '../data/flights/flight_5m.csv' method uniform size 0.05"
            )
    ...
    def workload(
        self,
        mdl_name,
        result2file: str = "experiments/flights/results/mdn1m/",
        n_jobs=1,
    ):
        ...
        self.sql_executor.execute(
            "SELECT SUM(taxi_out) FROM " + mdl_name + " where 1200<distance<=1300"
        )
...
if __name__ == "__main__":
    ...
    mdl_name = "flight_one_model1"
    query1.build_one_model(mdl_name, encoder="binary")
    # query1.build_one_model("flight_one_model_embedding",encoder="embedding")
    query1.workload(mdl_name, result2file="flights/results/range/")
