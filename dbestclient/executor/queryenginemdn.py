...
from dbestclient.ml.integral import (
    approx_avg,
    approx_count,
    approx_integrate,
    approx_sum,
...
class MdnQueryEngine(GenericQueryEngine):
    ...
    def approx_avg(self, x_min, x_max, groupby_value, runtime_config):
        ...
        def f_pRx(*args):
            return (
                self.kde.predict([[groupby_value]], args[0], b_plot=False)
        ...
        if runtime_config["b_use_integral"]:
            a = integrate.quad(
                ...
                x_min,
                x_max,
                epsabs=self.config["epsabs"],
                epsrel=self.config["epsrel"],
            )[0]
            b = integrate.quad(
                f_p,
                x_min,
    ...
    def approx_sum(self, x_min, x_max, groupby_value, runtime_config):
        ...
        def f_pRx(*args):
            return (
                self.kde.predict([[groupby_value]], args[0], b_plot=True)
                * self.reg.predict(np.array([[args[0], groupby_value]]))[0]
            )
        ...
        if runtime_config["b_use_integral"]:
            ...
            result = approx_integrate(f_pRx, x_min, x_max) * float(
                self.n_total_point[str(int(groupby_value))]
            )
    ...
    def predict(
        self, func, x_lb=None, x_ub=None, groupby_value=None, runtime_config=None
    ):
        if func.lower() == "count":
    ...
    def predicts(
        ...
        if time2exclude_from_multiprocessing is not None:
            ...
            print(
                "should reduce time {} from the query response time.".format(
                    (
                        t_after_multiple_processing - time2exclude_from_multiprocessing
                    ).total_seconds()
                )
            )
        ...
        if self.config.get_config()["accept_filter"]:
            ...
            results = {
                key: results[key]
                for key in results
                if float(key) >= filter_dbest[0] and float(key) <= filter_dbest[1]
            }
            ...
            if n_jobs == 1:
                ...
                if len(groups[0].split(",")) == 1:  # 1d group by
                    ...
                    scaling_factor = np.array(
                        [self.n_total_point[key] for key in groups]
                    )
                else:
                    # print(groups[0].split(","))
                    # print("n-d")
                ...
                if runtime_config["slaves"].is_empty():
                    ...
                    for sub_group in group_chunks:
                        ...
                        i = pool.apply_async(
                            ...
                            (
                                ...
                                x_ub,
                                x_categorical_conditions,
                                runtime_config_process,
                                sub_group,
                                filter_dbest,
                                time2exclude_from_multiprocessing,
                            ),
                        )
                        instances.append(i)
                    ...
                    for sub_group, host in zip(group_chunks, hosts):
                        ...
                        query = dict(
                            ...
                            x_lb=x_lb,
                            x_ub=x_ub,
                            x_categorical_conditions=x_categorical_conditions,
                            runtime_config=runtime_config_process,
                            sub_group=sub_group,
                            filter_dbest=filter_dbest,
                            mdl_name=self.mdl_name + runtime_config["model_suffix"],
                        )
                        i = pool.apply_async(
                            app_client.run,
                            (hosts[host].host, hosts[host].port, "select", query),
                        )
                        instances.append(i)
...
class MdnQueryEngineGoGs:
    ...
    def fit(
        ...
        yheader: str,
        runtime_config: dict,
    ):  # n_per_group: int = 10, n_mdn_layer_node=10, encoding = "onehot", b_grid_search = True
        ...
        if isinstance(self.group_keys[0], tuple):  # n-d group by  #tuple
            ...
            group_keys_float = sorted(group_keys_float, key=lambda k: [k[1], k[2]])
            ...
            self.group_keys_chunk = [
                sorted_group_keys[i : i + n_per_group]
                for i in range(0, len(sorted_group_keys), n_per_group)
            ]
            # print("*"*100)
            ...
            groups_chunk = [
                pd.concat(
                    [grouped.get_group(tuple(grp.split(","))) for grp in sub_group]
                )
            ...
            self.group_keys = [
                k for _, k in sorted(zip(group_keys_float, self.group_keys))
            ]
            self.group_keys_chunk = [
                self.group_keys[i : i + n_per_group]
                for i in range(0, len(self.group_keys), n_per_group)
            ]
            groups_chunk = [
                pd.concat([grouped.get_group(grp) for grp in sub_group])
                for sub_group in self.group_keys_chunk
            ]
            # print("self.group_keys_chunk", self.group_keys_chunk)
            # print("groups_chunk")
        ...
        # print(n_total_point)
        for index, [chunk_key, chunk_group] in enumerate(
            zip(self.group_keys_chunk, groups_chunk)
        ):
            ...
            n_total_point_chunk = {
                k: n_total_point[k] for k in n_total_point if k in chunk_key
            }
    ...
    def serialize2warehouse(self, warehouse, runtime_config):
        ...
        with open(
            warehouse + "/" + self.init_pickle_file_name(runtime_config), "wb"
        ) as f:
            dill.dump(self, f)
...
class MdnQueryEngineXCategorical(GenericQueryEngine):
    ...
    def fit(
        ...
        for categorical_attributes in total_points:
            print("start training  sub_model " + str(idx) + " for " + mdl_name + "...")
            idx += 1
            # print("total_points", total_points)
            ...
            if not self.config.get_config()["b_use_gg"]:
                kdeModelWrapper = KdeModelTrainer(
                    mdl_name,
                    origin_table_name,
                    usecols["x_continous"][0],
                    usecols["y"],
                    groupby_attribute=usecols["gb"],
                    groupby_values=list(total_points[categorical_attributes].keys()),
                ...
                qe_mdn = MdnQueryEngineGoGs(config=self.config.copy()).fit(
                    data[categorical_attributes],
                    usecols["gb"],
                    total_points[categorical_attributes],
                    mdl_name,
                    origin_table_name,
                    usecols["x_continous"][0],
                    usecols["y"],
                ...
                for value in distinct_values:
                    if meet_condition(value, x_categorical_conditions[2][col]):
                        ...
                        for key_item in self.x_categorical_columns:
                            # print(key_item)
                            if key_item in cols:
                                key.append(keys[cols.index(key_item)].replace("'", ""))
                            else:
                                key.append(value)
                        # print(key)
                        key = ",".join(key)
...
class MdnQueryEngineXCategoricalOneModel(GenericQueryEngine):
    ...
    def predicts(
        ...
        groups = [
            [item] + x_categorical_conditions[1] for item in groups_no_categorical
        ]
        ...
        if n_jobs == 1:
            ...
            if len(groups[0].split(",")) == 1:  # 1d group by
                ...
                # print("1d")
                scaling_factor = np.array(
                    [self.n_total_points[key][k] for k in groups_no_categorical]
                )
            else:
                # print(groups[0].split(","))
                # print("n-d")
